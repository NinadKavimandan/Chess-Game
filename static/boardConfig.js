...
function initBoard() {
	...
	$('.pieces').draggable({
		...
		start: function(event) {
			...
			//console.log(""+element);
			if(element.substring(0,1) != turnColr[turn] || turn != mcol) return false;
	...
	$('.cell').droppable({
		...
		drop: function(event,ui) {
			...
			var place = this.id;
			//queryExecutor(""+place);
			var pref = getElePrefix(element);
			if(findPiece(place) != element)
			{
				if(findPiece(place) != "none") pref = pref + "x";
				killIt(place);
			}
			else turn++;
			pieceLoc[element] = place;
